...
Array.from(moviesCheckboxes).forEach(
...
if (screen.width < 768) {
    videoControlButtonIcon = document.querySelector("i.video-control-button");
...
videoControlButton.addEventListener("click", function () { onVideoClick(); });

bindVideosEnd();
function bindVideosEnd() {
    var videos = document.getElementsByTagName("video");
    Array.from(videos).forEach(
        function (element, index, array) {
...
function bindCarouselLinks() {
    Array.from(document.getElementsByClassName("carousel-link")).forEach(
        function (element, index, array) {
...
function stopAllRunningVideos() {
    Array.from(runningVideos).forEach(
...
function toggleMoviesCaption() {
    ...
    if (carouselIndicators.style.visibility != "visible") {
        ...
        Array.from(document.getElementsByClassName("carousel-caption")).forEach(
            function (caption, index, array) {
...
function bindPlayTrailerLinks() {
    Array.from(document.getElementsByClassName("trailer-indicator")).forEach(
        function (element, index, array) {
            element.addEventListener("click", function () {
